...
function Game.new(GameFrame: Frame, Player: player.Player2d, Map: map.Map): Game
	...
	local function showAnimation(animationName: string)
		if
			...
			and self.Player.CurrentAnimation.AnimationRunning ~= self.Player.Animations[animationName]
		then
	...
	local Up = cooldown.new(cooldownTime, function()
		showAnimation("WalkUp")

		TweenService:Create(self.Map.Image.ImageInstance, TweenInfo.new(cooldownTime), {
			["Position"] = UDim2.new(
				self.Map.Image.Position.X,
				UDim.new(self.Map.Image.Position.Y.Scale, self.Map.Image.Position.Y.Offset + self.Player.WalkSpeed)
			),
		}):Play()
	...
	local Down = cooldown.new(cooldownTime, function()
	...
	local Left = cooldown.new(cooldownTime, function()
		showAnimation("WalkLeft")

		TweenService:Create(self.Map.Image.ImageInstance, TweenInfo.new(cooldownTime), {
			["Position"] = UDim2.new(
				UDim.new(self.Map.Image.Position.X.Scale, self.Map.Image.Position.X.Offset + self.Player.WalkSpeed),
	...
	local Right = cooldown.new(cooldownTime, function()
